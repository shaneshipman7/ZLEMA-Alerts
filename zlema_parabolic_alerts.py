...
# ─────────────────────────────────────────────
def get_best_timeframe(ticker: str, zlema_period=15, atr_period=14, min_streak=3):
    try:
    ...
    for tf_name, config in timeframes.items():
        try:
            ...
            if score > best_score and active_streak >= min_streak - 1:
                best_score = score
                best_tf = tf_name
                best_data = {
                    'up_streak': up_streak,
                ...
                print(f"  {ticker} → Best TF so far: {tf_name} | Streak: {active_streak} | Score: {score:.1f}")
        except Exception as e:
            print(f"  ❌ Error on {ticker} {tf_name}: {type(e).__name__}: {e}")
            continue
...
def main():
    ...
    print(f"\n🔍 Scanning {len(tickers)} tickers with min-streak={args.min_streak}...")

    bull_alerts = []
    bear_alerts = []

    for ticker in tickers:
        best_tf, data = get_best_timeframe(ticker, min_streak=args.min_streak)
        if not best_tf or not data:
            print(f"  ⚠️ No valid timeframe found for {ticker}")
            continue

        entry = {'ticker': ticker, 'tf': best_tf, **data}

        if data['bull_flip'] or data['up_streak'] >= args.min_streak:
            bull_alerts.append(entry)
        if data['bear_flip'] or data['down_streak'] >= args.min_streak:
